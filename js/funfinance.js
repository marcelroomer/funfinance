...
function toDate(number) {
    return Highcharts.dateFormat('%H:%M:%S', number)
}

function lastPoint() {
    var chart = $('#container').highcharts();
    //var l = chart.series[1].points.length;
    //var p = chart.series[1].points[l-1];
    var l = chart.series[1].data.length;
    var p = chart.series[1].data[l-1];
    return p
}

function lastBar() {
    var chart = $('#container').highcharts();
    var l = chart.series[2].data.length;
    if (l>0){
        var b = chart.series[2].data[l-1];
        //console.log(toDate(b.x));
    }
}

function updateYextreme() {
    var mychart;
    mychart = $('#container').highcharts();
    lastpoint = lastPoint();  // from ticker
    mychart.yAxis[0].setExtremes(lastpoint.y-0.05, lastpoint.y+0.05);
}
...
function showYasExtremes() {
    ...
    chart.renderer.label(
            ...
            'dataMax: '+ toDate(extremes.dataMax) +'<br/>'+
            'min: '+ toDate(extremes.min) +'<br/>'+
